...
<?php
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT avatar FROM `200` WHERE user_id =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("../../../department/computersci/students/level/200lvl/image/".$imgRow['avatar']);
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM `200` WHERE user_id =:uid');
...
<?php
	...
	$stmt = $DB_con->prepare('SELECT user_id, user_name, user_email, avatar FROM `200` ORDER BY user_id DESC');
	$stmt->execute();
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			<div class="col-xs-3">
                ...
                <a title="Click to zoom-in and zoom-out" class="zoom"  href="../../../department/computersci/students/level/200lvl/image/<?php echo $row['avatar']; ?>" >
                    <img src="../../../department/computersci/students/level/200lvl/image/<?php echo $row['avatar']; ?>" class="img-rounded" width="250px" height="250px" /></a>
				<p class="page-header">
				<span>
