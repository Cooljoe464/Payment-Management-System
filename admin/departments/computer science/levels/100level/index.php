...
<?php
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT avatar FROM `users` WHERE id =:uid ');
...
<div class="container">
	...
	<div class="page-header">
       <ul class="nav navbar-nav navbar-right">
        ...
        <a class="btn btn-danger" type="submit" onclick="return confirm('Are you sure you want to delete ?')" href="delete_id=<?php$stmt= $DB_con->prepare('Delete * from `users`');
        $stmt->execute();
...
<?php
	...
	$stmt = $DB_con->prepare('SELECT * FROM `users` WHERE levels = 1 ORDER BY id DESC');
	$stmt->execute();
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			<div class="col-xs-3">
				...
				<p class="page-header" title="Click to view">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="view.php?view_id=<?php echo $row['id']; ?>
                        " style="color: whitesmoke;"title="Click to view" class="btn btn-success"><?php echo $user_name; ?>
                    </a><?php

                    ?></p><br>
                </form>
                <a title="Click to zoom-in and zoom-out" class="zoom"  href="../../../../../image/<?php echo $row['avatar']; ?>" >
                    <img  src="../../../../../image/<?php echo $row['avatar']; ?>" class="img-rounded" width="250px" height="250px" /></a>
				<p class="pagination">
				<span>
				<a class="btn btn-info" href="editform.php?edit_id=<?php echo $row['id']; ?>" title="click to edit" onclick="return confirm('sure you want to edit ?')"><span class="glyphicon glyphicon-pencil"></span> Edit</a>
