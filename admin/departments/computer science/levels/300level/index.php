...
<?php
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT avatar FROM `300` WHERE user_id =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("../../../department/computersci/students/level/300lvl/image/".$imgRow['avatar']);
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM `300` WHERE user_id =:uid');
...
<div class="container">
	...
	<div class="page-header">
       <ul class="nav navbar-nav navbar-right">
        ...
        <a class="btn btn-danger" type="submit" onclick="return confirm('Are you sure you want to delete ?')" href="delete_id=<?php$stmt= $DB_con->prepare('Delete * from `100`');
        $stmt->execute();
...
<?php
	...
	$stmt = $DB_con->prepare('SELECT user_id, user_name, user_email, avatar FROM `300` ORDER BY user_id DESC');
	$stmt->execute();
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			<div class="col-xs-3">
                ...
                <a title="Click to zoom-in and zoom-out" class="zoom"  href="../../../department/computersci/students/level/300lvl/image/<?php echo $row['avatar']; ?>" ><img src="../../../department/computersci/students/level/300lvl/image/<?php echo $row['avatar']; ?>" class="img-rounded" width="250px" height="250px" /></a>
				<p class="page-header">
				<span>
