    ...
    if ($_POST['txt_upass'] == $_POST['confirm_password']) {
        ...
        if (in_array($imgExt, $valid_extensions)) {
            ...
            if ($imgSize < 2000000) {
                ...
                $side = "INSERT INTO `users` (dateofbirth, phonenumber, levels,  hostel, sponsor, sponsorphonenumber, first_name, last_name, user_email, user_name , password, avatar, code, active) 
                VALUES ('{$date_of_birth}','{$phone_number}','{$levels}','{$hostel_address}','{$name_of_sponsors}','{$phone_number_of_sponsors}','{$firstname}','{$lastname}','{$email}','{$username}','{$password}','{$avatar_path}','{$code}','{$active}')";
    ...
    <div class="container" style="background-size: contain; background-color: #37474f;">
        ...
        <form method="post" action="100.php" class="form" enctype="multipart/form-data" >
            ...
            <div class="form-group">
                <div class="cols-sm-10">
                    <div class="input-group">
                        <input value="2" title="" type="hidden" class="form-control" name="levels"  />
                    </div>
                </div>
            </div>
