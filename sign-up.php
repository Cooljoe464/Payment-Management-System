<?php
session_start();
require_once('students/class.user.php');
$user = new USER();

if($user->is_loggedin()!="")
{
	$user->redirect('students/index.php');
}
...
if(isset($_POST['btn-signup']))
{
	...
	if($uname=="")	{
		$error[] = "provide matric number !";
	}
	else if ($fname == "") {
		$error[] = "provide first name";
	}
	else if($umail=="")	{
		$error[] = "provide your matric number !";
	}
	else if(!filter_var($umail, FILTER_VALIDATE_EMAIL))	{
	    $error[] = 'Please enter a valid email address or matric number !';
	}
	else if($upass=="")	{
	...
	else
	{
		try
		{
			$stmt = $user->runQuery("SELECT user_name, user_email, usermat FROM users WHERE user_name=:uname OR user_email=:umail");
			...
			{
				if($user->register($uname,$umail,$upass,$fname,$lname,$dob,$department,$level,$phone,$hostel,$tuition_amt,$name_of_spon,$phone_num_spon,$gender)){
            ...
            <form class="" method="post" action="#">
                ...
                else if(isset($_GET['joined']))
                {
                    ...
                    <div class="alert alert-info">
                        <i class="glyphicon glyphicon-log-in form-control-feedback"></i> &nbsp; Successfully registered <a href='students/index.php'>login</a> here
                    </div>
                    <?php
                }
                ?>
                ...
                <div class="form-group">
                    <label for="username" class="cols-sm-2 control-label">Sponsor's Name</label><!--Also known as the username-->
                    <div class="cols-sm-10">
                        <div class="input-group">
                            <span class="input-group-addon"><i class="fa fa-users fa" aria-hidden="true"></i></span>
                ...
                <div class="form-group">
                    <label for="username" class="cols-sm-2 control-label">Sponsor's Phone Number</label><!--Also known as the username-->
                    <div class="cols-sm-10">
                        <div class="input-group">
                            <span class="input-group-addon"><i class="fa fa-users fa" aria-hidden="true"></i></span>
                            <input type="text" class="form-control" name="txt_phone_num_spon" required placeholder="Phone number of sponsor" value="<?php if(isset($error)){echo $phone_num_spon;}?>" />
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <hr>
                    <div class="form-group ">
                        <label>have an account ! <a href="students/index.php">Log In</a></label>
                    </div>
